...
- **Memory** — distilled, persistent insights from conversations or user input
...
- Vector search using Kùzu HNSW indexes with FastEmbed (384-dimensional)
...
ok graph cypher "MATCH (m:MemoryNote {text: CONTAINS 'attention'})-[:DerivedFrom]->(c:Chunk)-[:HAS_CHUNK]->(d:Doc) RETURN d.path, c.text LIMIT 5"
...
- Implement:
  ...
  - `index`, `sync`, `doctor`
  - Kùzu schema + embeddings (sentence-transformers)
